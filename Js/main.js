...
const regexConteudo = /^[A-Za-z0-9\s]{10,}$/;
const regexAutoria = /^[A-Za-z\s]{3,20}$/;
...
async function manipularSubmissaoFormulario(event) {
  ...
  if (!validarConteudo(conteudoSemEspaco)) {
    alert(
      "O conteúdo deve ter pelo menos 10 caracteres, com letras e espaços."
    );
    return;
  }

  if (!validarAutoria(autoriaSemEspaco)) {
    alert("A autoria deve ter entre 3 e 15 caracteres, com letras e espaços.");
    return;
  }
